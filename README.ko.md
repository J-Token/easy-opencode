...
OpenCode 플러그인으로, **LSP 도구 11개** + **AST-grep 도구 2개**를 제공합니다.

에이전트 오케스트레이션이나 백그라운드 작업은 제공하지 않습니다.
...
`npx @j-token/easy-opencode`를 실행하면 `~/.config/opencode/opencode.jsonc`(우선) 또는 `~/.config/opencode/opencode.json`에 내장된 프로바이더 프리셋을 병합합니다.
